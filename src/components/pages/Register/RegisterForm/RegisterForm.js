  ...
  const onSubmit = useCallback(
    async (e) => {
      ...
      if (!errorOccurred) {
        ...
        try {
          const result = await userCredentialDetailsService.register(
            data
          );

          const loginResult = await userCredentialDetailsService.login(
            {"username": data["email"], "password": data["password"]}
          );

          clearInitialFormValuesMessages(FORM_KEYS, INITIAL_FORM_VALUES);

          await updateAuthentication(loginResult);
        } catch (err) {
